...
type Session struct {
	...
	ikeStateMu sync.RWMutex // Packet crypto and IKE SA switch must use one consistent snapshot.
	localResponder bool // Role in the current IKE SA, independent of request/response.
	retiredMu sync.Mutex
	retiredIKE []*retiredIKESA
	...
	ikeIntegID uint16
	ikeIsAEAD  bool
	ikeProposal *ikev2.Proposal // negotiated transforms, without an SPI
