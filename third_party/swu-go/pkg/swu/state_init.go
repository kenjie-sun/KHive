...
func (s *Session) buildIKESAInitPacket() ([]byte, error) {
	...
	proposals, err := ikev2.ConfiguredProposals(s.cfg.IKEProposals, ikev2.ProtoIKE, nil)
	if err != nil {
		return nil, err
	}
...
func (s *Session) handleIKESAInitResp(data []byte) error {
		...
		case *ikev2.EncryptedPayloadNotify:
			...
			if v.NotifyType == 17 { // INVALID_KE_PAYLOAD
				if len(v.NotifyData) != 2 {
					return errors.New("invalid INVALID_KE_PAYLOAD notification")
				}
				return fmt.Errorf("ePDG requested unsupported DH group %d (INVALID_KE_PAYLOAD); configured DH group is 14", binary.BigEndian.Uint16(v.NotifyData))
			}
	...
	if saPayload == nil || len(saPayload.Proposals) == 0 || kePayload == nil || noncePayload == nil {
		return errors.New("SA_INIT 响应中缺少强制性载荷")
	}
	if kePayload.DHGroup != ikev2.MODP_2048_bit {
		return fmt.Errorf("ePDG returned unexpected DH group %d", kePayload.DHGroup)
	}
	...
	s.Logger.Info("ePDG_SA_INIT: IKE SA 算法协商成功",
	...
	// 计算共享密钥
	s.ikeProposal = copySAProposal(selProp, 1, nil)
