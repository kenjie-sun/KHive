...
type PolicyInput struct {
	LocalSPIc, LocalSPIs   uint32
	LocalPortC, LocalPortS int
	LocalIP                net.IP
	RemoteIP               net.IP
...
func NewPolicy(in PolicyInput) (Policy, error) {
	...
	if in.Mech.SPIc == 0 || in.Mech.SPIs == 0 || in.LocalSPIc == 0 || in.LocalSPIs == 0 {
		return Policy{}, errors.New("ipsec3gpp: spi-c and spi-s are required")
	}

	ports := portPair{in.LocalPortC, in.LocalPortS, in.Mech.PortC, in.Mech.PortS}
	for _, port := range []int{ports.localC, ports.localS, ports.remoteC, ports.remoteS} {
		if port < 1 || port > 65535 {
			return Policy{}, errors.New("ipsec3gpp: invalid negotiated port")
		}
	}
