...
func (t *Transport) TransformInbound(packet []byte) ([]byte, error) {
	...
	if parsed.nextHeader != ipProtoESP {
		if parsed.nextHeader == ipProtoTCP && ipEqual(parsed.dst, t.policy.LocalIP) {
			for _, flow := range t.outbound {
				if parsed.dstPort == flow.flow.LocalPort {
					return nil, errors.New("ipsec3gpp: unprotected packet on secure SIP port")
				}
			}
		}
		t.passthroughPackets.Add(1)
	...
	if !ipEqual(parsed.src, t.policy.RemoteIP) || !ipEqual(parsed.dst, t.policy.LocalIP) || len(plain) < 20 || int(binary.BigEndian.Uint16(plain[:2])) != flow.flow.RemotePort || int(binary.BigEndian.Uint16(plain[2:4])) != flow.flow.LocalPort {
		return nil, errors.New("ipsec3gpp: inbound traffic selector mismatch")
	}
	// Do not advance the replay window using unauthenticated sequence numbers.
...
func isIPv6ExtensionHeader(proto uint8) bool {
	switch proto {
	// ESP is the protected payload itself, not a clear-text extension header.
	case 0, 43, 60, 135:
		return true
	default:
		return false
...
func cloneSAForInbound(template *ipsec.SecurityAssociation, spi uint32) *ipsec.SecurityAssociation {
	...
	return ipsec.NewSecurityAssociationCBC(spi, template.EncryptionAlg, template.EncryptionKey, template.IntegrityAlg2, template.IntegrityKey)
}
...
func decapsulateTransport(packet []byte, sa *ipsec.SecurityAssociation) ([]byte, error) {
	...
	if plaintext[len(plaintext)-1] != ipProtoTCP {
		return nil, errors.New("ipsec3gpp: unexpected protected next header")
	}
	if len(plaintext) < 2+padLen {
		return nil, errors.New("ipsec3gpp: invalid padding length")
	}
	for i := 0; i < padLen; i++ {
		if plaintext[len(plaintext)-2-padLen+i] != byte(i+1) {
			return nil, errors.New("ipsec3gpp: invalid ESP padding")
		}
	}
