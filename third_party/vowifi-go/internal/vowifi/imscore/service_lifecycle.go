...
func (s *Service) Start(ctx context.Context) error {
	...
	s.ipsecInstalled = reg.transport != nil
	s.pcscf = winningPCSCF
	s.localAddr = s.cfg.LocalIP.String()

	if reg.secureConn == nil {
		return fmt.Errorf("IMS registration did not provide a signaling connection")
	}
	if reg.transport != nil {
		rt, err := startTransportRuntime(lifecycleCtx, s.cfg, swu, reg.ipsecPolicy, reg.transport, reg.secureConn, reg.portSListener)
		if err != nil {
			_ = reg.secureConn.Close()
			return err
		}
...
func (s *Service) attachMessaging(ctx context.Context, winningPCSCF string, reg *registerResult) error {
	voiceCfg := voiceclient.Config{
		...
		SkipRegister:    true,
		SWUDialer:       s.swu,
		InitialConn:     reg.secureConn,
		SecurityVerify:  reg.verifyHeader,
		ServiceRoutes:   reg.serviceRoutes,
		RefreshRequest:  reg.request,
	}
	voiceCfg.RegisterExpiry = time.Duration(reg.expiresSeconds) * time.Second
	voiceCfg.SIPInstanceURN = reg.sipInstance
	if a, ok := reg.secureConn.LocalAddr().(*net.TCPAddr); ok {
		voiceCfg.LocalPort = a.Port
	}
	voiceCfg.PCSCFAddr = reg.secureConn.RemoteAddr().String()
	if s.transportRuntime != nil {
		voiceCfg.TCPListener = s.transportRuntime.portSListener
