...
func installIPSecFromChallenge(cfg Config, state *registerState, res *sip.Response) error {
	...
	pol, err := ipsec3gpp.NewPolicy(ipsec3gpp.PolicyInput{
		LocalIP:   cfg.LocalIP,
		LocalSPIc: state.spiC, LocalSPIs: state.spiS, LocalPortC: state.portC, LocalPortS: state.portS,
		RemoteIP: rip,
		Mech:     mech,
		CK:       state.ck,
		IK:       state.ik,
...
// ESP is installed on the IP dataplane, below TCP, including SYN/ACK packets.
func dialSecureRegisterConn(ctx context.Context, cfg Config, swuTCP voiceclient.SWUTCPDialer, state registerState) (net.Conn, error) {
	installer, ok := swuTCP.(voiceclient.PacketTransformInstaller)
	if !ok {
		return nil, fmt.Errorf("imscore: IPsec requires a packet-transform capable SWu dialer")
	}
	installer.SetPacketTransform(state.transport)
	return swuTCP.DialContextTCP(ctx, cfg.LocalIP, state.ipsecPolicy.LocalPortC, net.IP(state.ipsecPolicy.RemoteIP), state.ipsecPolicy.RemotePortS)
}
...
func finalizeRegisterSuccess(cfg Config, state registerState, res *sip.Response) (*registerResult, error) {
	...
	if contact := res.Contact(); contact != nil {
		if raw, ok := contact.Params.Get("expires"); ok {
...
func computeAKAAuth(cfg Config, chal *digest.Challenge, req *sip.Request) (sim.AKAResult, string, error) {
	result, err := simauth.ComputeDigest(cfg.AKA, chal, digest.Options{Method: req.Method.String(), URI: req.Recipient.String(), Username: authorizationUsername(cfg)})
	if err != nil {
