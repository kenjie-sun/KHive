...
func newSWuDatagramTransport(tp transport.DatagramTransport, remoteAddr *net.UDPAddr, onFailure func(error)) *swuDatagramTransport {
...
func (t *swuDatagramTransport) readLoop() {
	...
	for {
		...
		if err != nil {
			...
			case <-t.stopping:
			default:
				logger.Warn("VoWiFi SWu proxy transport ended", logger.Err(err))
				if t.onFailure != nil {
					// Failure cancellation can synchronously stop this transport;
					// let readLoop finish before the callback waits on Stop.
					go t.onFailure(fmt.Errorf("SWu proxy receive failed: %w", err))
				}
			}
			return
		}
...
func buildSWuTransportFactory(proxy *ProxyConfig, onFailure func(error)) func(string, string) (externalswu.Transport, error) {
	...
	return func(local, remote string) (externalswu.Transport, error) {
		...
		return newSWuDatagramTransport(tp, remoteAddr, onFailure), nil
	}
}
