...
// Refresh on the established security association. A fresh challenge or failed
// connection ends readiness; the runtime supervisor rebuilds SWu/IMS and keys.
func (c *Client) reregisterLoop() {
	...
	for {
		...
		case <-c.failure:
			return
		case <-timer.C:
			ctx, cancel := context.WithTimeout(c.lifecycleCtx, registerTransactionTimeout)
			var err error
			if c.cfg.SkipRegister {
				err = c.refreshRegistration(ctx)
			} else {
				err = c.registerWithResync(ctx)
			}
			cancel()
			if err != nil {
				c.failSignaling(err)
...
func (c *Client) refreshRegistration(ctx context.Context) error {
	req := c.cfg.RefreshRequest
	if req == nil {
		return fmt.Errorf("missing established REGISTER for refresh")
	}
	req = req.Clone()
	...
	if res.StatusCode != 200 {
		return fmt.Errorf("IMS refresh requires registration restart: %d", res.StatusCode)
	}
	c.cfg.RefreshRequest = req
	c.mu.Lock()
	c.refreshes++
	...
	for {
		...
		case <-tx.Done():
			if err := c.Err(); err != nil {
				return nil, fmt.Errorf("signaling connection failed: %w", err)
			}
