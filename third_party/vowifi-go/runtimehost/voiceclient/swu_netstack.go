...
// Leave room for IMS ESP expansion inside the SWu tunnel.
const swuNetstackMTU = 1360
...
// PacketTransform protects IP packets below TCP. A single netstack owns the
// SWu receive stream; IMS ESP must never be embedded in TCP application data.
type PacketTransform interface {
	TransformOutbound([]byte) ([]byte, error)
	TransformInbound([]byte) ([]byte, error)
}

type PacketTransformInstaller interface{ SetPacketTransform(PacketTransform) }
...
func (n *swuNetstack) SetPacketTransform(t PacketTransform) {
	n.transformMu.Lock()
	n.transform = t
	n.transformMu.Unlock()
}

func (n *swuNetstack) packetTransform() PacketTransform {
	n.transformMu.RLock()
	defer n.transformMu.RUnlock()
	return n.transform
}
...
func (n *swuNetstack) inboundLoop() {
	for {
		...
		case packet, ok := <-n.dp.InnerPackets():
			...
			if transform := n.packetTransform(); transform != nil {
				var err error
				packet, err = transform.TransformInbound(packet)
				if err != nil {
					continue
				}
			}
...
func (n *swuNetstack) outboundLoop() {
	for {
		...
		if transform := n.packetTransform(); transform != nil {
			var err error
			payload, err = transform.TransformOutbound(payload)
			if err != nil {
				continue
			}
		}
