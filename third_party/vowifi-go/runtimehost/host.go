...
func (i *Instance) updateState(mut func(*State)) State {
	...
	defer i.mu.Unlock()
	mut(&i.state)
	// A pipeline cannot become ready again after its underlying tunnel ended.
	// Recovery creates a new Instance; late IMS callbacks belong to this one.
	if i.swuFailure != nil {
		applySWuFailure(&i.state, i.swuFailure)
	}
	i.state.Phase = runtimePhase(i.state)
	return i.state
}

func runtimePhase(s State) string {
	switch {
	case s.LastErrorClass != "":
		return "failed"
	case s.SMSReady:
		return "sms_ready"
	case s.IMSReady:
...
func Start(ctx context.Context, req StartRequest) (*Instance, error) {
	...
	imsPrivateID := resolveIMSPrivateID(req.Prepared, imsi)
...
func (i *Instance) runStagedPipeline(ctx context.Context, req StartRequest) {
	...
	tunnelCtx, cancel := context.WithCancel(pipelineCtx)
	defer cancel()
	i.mu.Lock()
	i.swuCancel = cancel
	i.mu.Unlock()
	...
	session := runtimecore.BeginSession(runtimecore.SessionConfig{DeviceID: i.deviceID, TraceID: i.traceID})
	runtimecore.AttachIMSService(session, svc, svc.Status, localIP.String(), winningPCSCF)
	i.mu.Lock()
	if i.stopped || i.swuFailure != nil || pipelineCtx.Err() != nil {
		i.mu.Unlock()
	...
	i.notifyObservers(ctx)
	// The pipeline owns the IMS lifetime. Returning here would run the deferred
	// pipelineCancel and close the newly registered TCP connections immediately.
	i.watchIMSService(pipelineCtx, svc)
}

type imsServiceLifetime interface {
	Done() <-chan struct{}
	Err() error
	Close(context.Context) error
}

func (i *Instance) watchIMSService(ctx context.Context, svc imsServiceLifetime) {
	select {
	case <-ctx.Done():
		return
	case <-svc.Done():
	...
	i.updateState(func(s *State) {
		...
		s.SMSReady = false
		s.TunnelReady = false
		s.LastErrorClass = "ims"
		s.LastError = fmt.Sprint(svc.Err())
		s.LastReason = "ims_connection_lost"
		s.UpdatedAt = time.Now()
	})
	i.notifyObservers(ctx)
	if cancel := i.swuCancel; cancel != nil {
...
func resolveIMSPrivateID(prepared *identity.PreparedSession, imsi string) string {
	if prepared == nil {
		return ""
	}
	if prepared.IMSIdentity.ActualSource == identity.IMSIdentitySourceISIM {
		return strings.TrimSpace(prepared.IMSIdentity.IMPI)
	}
	privateID, _ := voiceclient.BuildIMSIdentity(imsi, prepared.IMSRealm(), resolveIMSDomain(prepared), "imsi_home_domain")
	return privateID
}
