...
type SMSPart struct {
	// TargetURI is the service-centre PSI, distinct from the TPDU recipient.
	TargetURI string
...
// IncomingSMSReceiver persists an RP-DATA before the network is acknowledged.
// Protocol rejection returns an RP-ERROR with nil error; transient persistence
