...
func (i *Instance) startSWuSession(ctx context.Context, req StartRequest, epdgIP, epdgPort string) (swuSnapshot, net.IP, swuInnerDataplane, func(string, string) error, error) {
	...
	if factory := buildSWuTransportFactory(req.Proxy, func(err error) {
	...
	session.OnSessionDown = func() {
		i.failSWuSession(ctx, errors.New("SWu session reported tunnel loss"))
	}
	errCh := make(chan error, 1)
	go func() {
		err := session.Connect(ctx)
		// This goroutine remains responsible for the tunnel after OnReady has
		// returned to the IMS pipeline. Internal SWu cancellation does not cancel
		// ctx, so unexpected exits are distinguishable from an explicit Stop.
		if ctx.Err() == nil {
			if err == nil {
				err = errors.New("SWu session exited")
			}
			i.failSWuSession(ctx, err)
		}
		errCh <- err
