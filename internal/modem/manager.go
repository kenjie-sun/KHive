...
type commandRequest struct {
	...
	silent       bool
	highPriority bool
	finishOnRDY  bool // SIM reactivation only; RDY requires fresh state readback.
...
type Manager struct {
	...
	cfunTrace atomic.Pointer[cfunTrace]
	...
	atSequenceMu sync.RWMutex // Protect multi-command SIM file selection from other AT commands.
...
// SetPDUCallback lets the application commit raw fragments before storage deletion.
func (m *Manager) SetPDUCallback(cb func(string) error) {
	m.infoMu.Lock()
	m.pduCallback = cb
	m.infoMu.Unlock()
}
func (m *Manager) persistPDU(raw string) (bool, error) {
	m.infoMu.RLock()
	cb := m.pduCallback
	m.infoMu.RUnlock()
	if cb == nil {
...
func (m *Manager) handleCommand(req commandRequest) {
	trace := newCFUNTrace(req.cmd)
	if trace != nil {
		m.cfunTrace.Store(trace)
		defer func() { m.cfunTrace.CompareAndSwap(trace, nil); trace.report(m.cfg.ID, req.cmd) }()
	}
	startTime := time.Now()
	...
	for {
		...
		case msg := <-m.rxChan:
			...
			if req.finishOnRDY && req.cmd == "AT+CFUN=1" && line == "RDY" {
				...
				req.errChan <- errSIMFunctionRestarted
				return
			}
			...
			if line == "OK" {
				...
				req.respChan <- "> "
				break RespLoop
			} else if isSolicitedATResponse(req.cmd, line) {
				// 查询结果属于本次事务，不能触发重启、插拔卡等异步副作用。
				fullResponse = append(fullResponse, line)
...
func (m *Manager) readAndProcessSMSFromStorage(storage, index string) {
	...
	if handled, err := m.persistPDU(pduHex); handled {
		if err != nil {
			logger.Warn("短信持久化失败，保留模组存储以便重试", "device", m.cfg.ID, "err", err)
			return
		}
		_, _ = m.ExecuteAT("AT+CMGD="+index, 3*time.Second)
		return
	}
	// 解码 PDU
...
func (m *Manager) executeAT(cmd string, timeout time.Duration, silent, highPriority bool) (string, error) {
	m.atSequenceMu.RLock()
	defer m.atSequenceMu.RUnlock()
	return m.executeATInSequence(cmd, timeout, silent, highPriority)
}

// Caller holds atSequenceMu, either for one command or a complete SIM transaction.
func (m *Manager) executeATInSequence(cmd string, timeout time.Duration, silent, highPriority bool, finishOnRDY ...bool) (string, error) {
	if !m.HasATPort() {
	...
	req.finishOnRDY = len(finishOnRDY) > 0 && finishOnRDY[0]
...
func (m *Manager) CheckAllSMS() {
	...
	records, err := m.smsListPDURecords()
	if err != nil {
		logger.Warn("检查短信失败", "device", m.cfg.ID, "err", err)
		return
	}
	for _, record := range records {
		handled, err := m.persistPDU(record.PDU)
		if err != nil {
			logger.Warn("短信持久化失败，保留模组存储以便重试", "device", m.cfg.ID, "index", record.Index, "err", err)
			continue
		}
		if !handled {
			sender, content, at := m.decodePDU(record.PDU)
			if m.smsCallback != nil && content != "" {
				m.smsCallback(sender, content, at)
			}
		}
		// Delete only entries read in this snapshot; newly arrived SMS are untouched.
		if _, err := m.ExecuteAT("AT+CMGD="+record.Index, 3*time.Second); err != nil {
			logger.Warn("删除已保存短信失败", "device", m.cfg.ID, "index", record.Index, "err", err)
		}
	}
}
