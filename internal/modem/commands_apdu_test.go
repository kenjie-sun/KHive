...
// A metadata CRSM changes the selected application on channel 0. It must not
// land between SELECT MF and SELECT EF_DIR, even without an installed arbiter.
func TestKHiveAIDDirectoryExcludesConcurrentMetadata(t *testing.T) {
	for _, withArbiter := range []bool{false, true} {
		t.Run(fmt.Sprint(withArbiter), func(t *testing.T) {
			m := newRunningTestManager(t)
			if withArbiter {
				m.SetAPDUArbiter(apduarbiter.New("test", apduarbiter.Options{}))
			}
			aidDone := make(chan error, 1)
			go func() {
				aid, _, err := m.ResolveSIMAuthAID("usim", "A0000000871002")
				if err == nil && aid != "A0000000871002FF49FF0189" {
					err = fmt.Errorf("unexpected AID")
				}
				aidDone <- err
			}()
			mf := <-m.cmdChan
			metadataDone := make(chan error, 1)
			go func() { _, err := m.readSIMTransparentEF(28486, 17); metadataDone <- err }()
			select {
			case req := <-m.cmdChan:
				req.respChan <- "ERROR"
				mf.respChan <- "ERROR"
				t.Fatalf("metadata interleaved while MF selection pending: %s", req.cmd)
			case <-time.After(30 * time.Millisecond):
			}
			mf.respChan <- "+CSIM: 4,\"9000\"\r\nOK"
			ef := <-m.cmdChan
			if ef.cmd != `AT+CSIM=14,"00a40004022f00"` {
				t.Fatalf("interleaved EF_DIR: %s", ef.cmd)
			}
			ef.respChan <- "+CSIM: 58,\"621482054221000A018002000A83022F008A01058B032F06019000\"\r\nOK"
			rec := <-m.cmdChan
			if rec.cmd != `AT+CSIM=10,"00b201040a"` {
				t.Fatalf("interleaved record: %s", rec.cmd)
			}
			rec.respChan <- "+CSIM: 42,\"61114F0CA0000000871002FF49FF01895001019000\"\r\nOK"
			if err := <-aidDone; err != nil {
				t.Fatal(err)
			}
			poll := <-m.cmdChan
			if !strings.HasPrefix(poll.cmd, "AT+CRSM=") {
				t.Fatalf("missing deferred metadata: %s", poll.cmd)
			}
			poll.respChan <- "+CRSM: 144,0,\"01\"\r\nOK"
...
func TestKHiveLogicalCSIMDoesNotRetryAmbiguousWrite(t *testing.T) {
	for _, response := range []string{"ERROR", "+CSIM: 4,\"6106\"\r\nOK"} {
		m := newRunningTestManager(t)
		commands := make(chan string, 1)
		go func() { req := <-m.cmdChan; commands <- req.cmd; req.respChan <- response }()
		_, err := m.TransmitLogicalAPDUViaCSIM(2, "82E2910003BF3300")
		if (err != nil) != (response == "ERROR") {
			t.Fatalf("unexpected error: %v", err)
		}
		if got := <-commands; got != "AT+CSIM=16,\"82E2910003BF3300\"" {
			t.Fatal(got)
		}
		select {
		case req := <-m.cmdChan:
			t.Fatalf("unexpected resend: %s", req.cmd)
		default:
		}
	}
}

func TestKHiveLogicalCSIMRejectsWrongChannelBeforeSending(t *testing.T) {
	m := newRunningTestManager(t)
	for _, tc := range []struct {
		channel int
		command string
	}{
		{0, "00C0000006"}, {20, "D0C0000006"}, {2, "81C0000006"}, {4, "83C0000006"}, {2, "not-hex"}, {2, "82"},
	} {
		if _, err := m.TransmitLogicalAPDUViaCSIM(tc.channel, tc.command); err == nil {
			t.Fatal("invalid channel accepted")
		}
	}
	select {
	case <-m.cmdChan:
		t.Fatal("sent invalid command")
	default:
	}
}

func TestKHiveLogicalCSIMSupportsExtendedChannel(t *testing.T) {
	m := newRunningTestManager(t)
	go func() { req := <-m.cmdChan; req.respChan <- "+CSIM: 4,\"9000\"\r\nOK" }()
	if response, err := m.TransmitLogicalAPDUViaCSIM(19, "CFC0000006"); err != nil || response != "9000" {
