...
type smsPDURecord struct{ Index, PDU string }

func parseSMSPDURecords(response string) []smsPDURecord {
	var out []smsPDURecord
	...
	for i, line := range lines {
		...
		value := strings.TrimSpace(strings.TrimPrefix(header, "+CMGL:"))
		index, _, _ := strings.Cut(value, ",")
		index, ok := normalizeSMSIndex(index)
		if !ok {
			continue
		}
		end := i + 1
		for end < len(lines) && !strings.HasPrefix(strings.TrimSpace(lines[end]), "+CMGL:") {
			end++
		}
		pdu, _ := extractSMSPDUAfterPrefix(strings.Join(lines[i:end], "\n"), "+CMGL:")
		if pdu != "" {
...
func (m *Manager) smsListPDURecords() ([]smsPDURecord, error) {
	response, err := m.ExecuteAT("AT+CMGL=4", 10*time.Second)
	if err != nil {
		return nil, err
	}
	return parseSMSPDURecords(response), nil
}
...
func (m *Manager) ResolveSIMAuthAID(app string, fallbackAID string) (string, string, error) {
	...
	// MF / EF_DIR / READ RECORD share channel 0 selection state. Keep both
	// APDU arbitration and AT command exclusion across the whole transaction;
	// CRSM metadata polling otherwise can select another DF between commands.
	lease, err := m.acquireAPDUTransportLease(8*time.Second, "vowifi_aid_directory", apduarbiter.APDUClassUSIMAKA, 0)
	if err != nil {
		return "", "sim_auth_aid_not_ready", err
	}
	if lease != nil {
		defer lease.Release()
	}
	...
	execute := func(cmd string, timeout time.Duration) (string, error) {
		return m.executeATInSequence(cmd, timeout, true, false)
	}
	aids, err := simaid.ReadDirectoryAIDs(func(apdu []byte) ([]byte, error) {
		if lease != nil && !lease.Touch() {
			return nil, fmt.Errorf("%w: SIM directory transaction lease expired", apduarbiter.ErrAPDUBusy)
		}
		respHex, err := m.transmitBasicAPDU(hex.EncodeToString(apdu), lease, execute)
		if err != nil {
			return nil, err
		}
...
func (m *Manager) TransmitAPDU(channel int, apduHex string) (string, error) {
	return m.transmitAPDU(channel, apduHex, false)
}

// TransmitLogicalAPDUViaCSIM sends once on an already opened logical channel.
// It must never retry or fall back after an ambiguous write response.
func (m *Manager) TransmitLogicalAPDUViaCSIM(channel int, apduHex string) (string, error) {
	if channel < 1 || channel > 19 {
		return "", fmt.Errorf("unsupported CSIM logical channel: %d", channel)
	}
	command, err := hex.DecodeString(apduHex)
	if err != nil || len(command) < 4 {
		return "", fmt.Errorf("invalid logical CSIM APDU")
	}
	claChannel := int(command[0] & 3)
	if command[0]&0x40 != 0 {
		claChannel = 4 + int(command[0]&0x0F)
	}
	if claChannel != channel {
		return "", fmt.Errorf("CSIM CLA channel does not match logical session")
	}
	return m.transmitAPDU(channel, apduHex, true)
}

func (m *Manager) transmitAPDU(channel int, apduHex string, csim bool) (string, error) {
	owner := "esim_apdu"
	...
	if csim {
		cmd = fmt.Sprintf("AT+CSIM=%d,\"%s\"", len(apduHex), apduHex)
	}
...
func (m *Manager) TransmitBasicAPDU(apduHex string) (string, error) {
	...
	return m.transmitBasicAPDU(apduHex, lease, m.ExecuteATSilent)
}

func (m *Manager) transmitBasicAPDU(apduHex string, lease *apduarbiter.Lease, execute func(string, time.Duration) (string, error)) (string, error) {
	send := func(currentAPDUHex string) (string, error) {
		...
		try := func(length int) (string, bool, error) {
			...
			resp, err := execute(cmd, 8*time.Second)
			if err != nil {
				return "", true, err
			}
