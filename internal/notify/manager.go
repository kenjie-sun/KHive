...
func (m *Manager) NotifySMSWithSource(deviceID, sender, content, source string, timestamp time.Time) {
	...
	identity, _ := json.Marshal([]string{deviceID, sender, content, timestamp.UTC().Format(time.RFC3339Nano)})
	id := fmt.Sprintf("sms-%x", sha256.Sum256(identity))
	...
	for _, ch := range m.channels {
		...
		go func() {
			...
			if withCtx, ok := ch.(contextualChannel); ok {
				err = withCtx.SendWithContext(ctx)
			} else {
				err = ch.Send(ctx.Text)
			}
			if err != nil {
				logger.Warn("通知渠道发送失败", "channel", ch.Name(), "event", ctx.Event, "notification_id", ctx.NotificationID, "sms_device", ctx.DeviceID, "err", err)
				return
			}
			logger.Info("通知渠道发送完成", "channel", ch.Name(), "event", ctx.Event, "notification_id", ctx.NotificationID, "sms_device", ctx.DeviceID)
		}()
	}
}
