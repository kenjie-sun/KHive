...
func TestHandleNewSMSQMIUsesIndicatedStorageAndDeletesSameStorage(t *testing.T) {
	...
	worker.state.Identity.IMSI = "test-qmi-sim"
...
func TestHandleRawSMSQMIRejectsDecodeFailure(t *testing.T) {
	initDevicePhoneNumberTestDB(t)
	stub := &qmiSMSCoreStub{}
	worker := &Worker{
		ID:          "wwan0",
	...
	if len(stub.ackCalls) != 0 {
		t.Fatalf("ackCalls=%d, want 0", len(stub.ackCalls))
	}
