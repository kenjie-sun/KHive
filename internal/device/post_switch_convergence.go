...
func convergePostSwitch(ctx context.Context, readiness postSwitchReadinessProvider, reloader any, opts postSwitchConvergenceOptions) postSwitchConvergenceResult {
	...
	for attempt := 1; attempt <= opts.IdentityAttempts; attempt++ {
		if ctx.Err() != nil {
			return postSwitchConvergenceResult{Degraded: true, Reason: "device_disconnected"}
		}
		probeCtx := ctx
		cancel := func() {}
		if opts.ProbeTimeout > 0 {
...
func (p *Pool) runPostSwitchConvergence(deviceID string, token uint64, worker *Worker, snapshot esimSwitchContext) postSwitchConvergenceResult {
	...
	if !ok {
		if reloader, ok := worker.Backend.(interface{ ReloadSIMAfterSwitch(context.Context) error }); ok {
			// Even refresh=true may expose the new QCCID before this AT
			// firmware reloads CIMI/USIM. Explicitly initialize the SIM once.
			ctx, cancel := context.WithTimeout(ctx, 50*time.Second)
			err := reloader.ReloadSIMAfterSwitch(ctx)
			cancel()
			if err != nil {
				logger.Warn("AT 切卡后 SIM 重新初始化失败", "device", deviceID, "err", err)
				return postSwitchConvergenceResult{Degraded: true, Reason: "at_sim_reload_failed"}
			}
			logger.Info("AT 切卡后 SIM 已重新初始化", "device", deviceID)
		}
		// AT backends have no QMI UIM service. Gate on fresh, matching SIM
		// identity instead; a successful profile APDU alone is insufficient.
		ready, err := p.refreshPostSwitchIdentity(deviceID, worker, snapshot)
		if err != nil || !ready {
			return postSwitchConvergenceResult{Degraded: true, Reason: "live_identity_not_ready"}
		}
		return postSwitchConvergenceResult{Ready: true, Reason: "live_identity_ready"}
	}
	...
	if worker.Config.ESIMSwitch.EventGatedConverge {
		...
		switch awaitReinitConverged(ctx, src, readiness, snapshot.TargetICCID, reinitWindow) {
		...
		case reinitTimeout:
			if power, ok := reloader.(postSwitchSIMPowerController); ok {
				...
				cancel()
				if err := triggerPowerCycleFallback(ctx, power, slot); err != nil {
	...
	result := convergePostSwitch(ctx, readiness, reloader, postSwitchConvergenceOptions{
	...
	logger.Info("eSIM 切卡后 SIM 收敛结果",
		...
		"slot", result.Slot)
	// Policy application belongs after the live identity has been committed
	// to the worker, otherwise CurrentICCID may still identify the old card.
	return result
}
