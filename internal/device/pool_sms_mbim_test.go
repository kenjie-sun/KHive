...
func TestDrainMBIMInboxReassemblesAndDeletesSegments(t *testing.T) {
	...
	w.state.Identity.IMSI = "test-mbim-sim"
	for seq, text := range map[int]string{1: "part1-", 3: "-part3", 4: "-part4"} {
		_, err := db.StoreIncomingSMS(context.Background(), db.IncomingSMSFragment{IMSI: "test-mbim-sim", Sender: sender, Content: text, TPDU: []byte{byte(seq)}, Timestamp: ts, Ref: concat.Ref, RefBits: concat.RefBits, Total: 4, Seq: seq, DCS: 0})
		if err != nil {
			t.Fatal(err)
		}
	}
