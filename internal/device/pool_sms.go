...
func (w *Worker) handleNewSMSQMI(storage uint8, index uint32) {
	...
	if err := w.processDecodedSMSQMI(sms); err != nil {
		logger.Warn("QMI 短信持久化失败，保留存储以便重试", "device", w.ID, "err", err)
		return
	}
...
func (w *Worker) handleNewSMSRawQMI(info qmicore.RawSMSIndication) {
	...
	if err != nil {
		...
		w.ackRawSMSQMI(info, "decode_failure", false)
		return
	}

	if err := w.processDecodedSMSQMI(sms); err != nil {
		w.ackRawSMSQMI(info, "storage_failure", false)
		return
	}
	w.ackRawSMSQMI(info, "processed", true)
}

func (w *Worker) ackRawSMSQMI(info qmicore.RawSMSIndication, reason string, success bool) {
	if !info.AckRequired {
		return
	}
	smsCore := w.smsQMICore()
	...
	defer cancel()
	if err := smsCore.AckRawSMS(ctx, info, success); err != nil {
...
func (w *Worker) processDecodedSMSQMI(sms *qmimanager.DecodedSMS) error {
	if sms == nil {
		return fmt.Errorf("missing decoded SMS")
	}
	...
	// The QMI provider exposes decoded fields, not the original TPDU or UDH
	// reference width. Use a separate fingerprint namespace and unknown RefBits.
	identity, _ := json.Marshal(struct {
		Sender, Text    string
		Timestamp       time.Time
		Ref, Total, Seq int
	}{sms.Sender, sms.Message, sms.Timestamp, sms.ConcatRef, sms.ConcatTotal, sms.ConcatSeq})
	total, seq := 1, 1
	if sms.IsConcat {
	...
	notifyStoredSMS(w.Pool, w.ID, "QMI", result)
	return nil
}
