...
func NewPool(cfg *config.Config) *Pool {
	...
	p.voWiFiHost().ConfigureRuntimeDependencies(p.GetVoiceGateway(), vowifiDeliveryStore{pool: p}, poolVoWiFiRuntimeDispatcher{pool: p})
...
func (p *Pool) RemoveWorker(deviceID string) error {
	...
	worker.EsimMgr.StopSwitchRecovery()
	if src := worker.currentSwitchEventSource(); src != nil {
		src.Close()
...
func (p *Pool) applyNetworkPreference(worker *Worker) error {
	...
	if worker.Config.NetworkEnabled {
		...
		if err := worker.StartNetwork(); err != nil {
			p.scheduleECMNetworkRecovery(worker)
			return err
		}
		p.refreshIPs(worker, true)
		return nil
...
func (p *Pool) startAllSynchronousLegacy() error {
	...
	for i := range p.cfg.Devices {
		...
		w.Backend = be
		w.initECMNetwork()
		onBeforeSwitch, onAfterSwitch, onSwitchFailed, onSwitchDegraded, onSwitchPhase := p.newESIMSwitchCallbacks(devCfg.ID, w)
		w.EsimMgr, err = newESIMManagerForWorker(w, qmiTransport, onBeforeSwitch, onAfterSwitch, onSwitchFailed, onSwitchDegraded, onSwitchPhase, p.esimSwitchObservers(w))
		if err != nil {
			if firstErr == nil {
...
func newESIMManagerForWorker(
	...
	observers ...esimSwitchObservers,
) (*esim.Manager, error) {
	if w == nil {
		return nil, fmt.Errorf("worker 不能为空")
	}

	var observer esimSwitchObservers
	if len(observers) > 0 {
	...
	mgr, err := esim.NewManager(esim.ManagerOptions{
		...
		ATUseCSIM:            w.Config.ESIMATCSIM,
		Modem:                w.Modem,
		Backend:              w.Backend,
		QMITransport:         qmiTransport,
		OnBeforeSwitch:       beforeWithOperation,
		OnAfterSwitch:        afterWithOperation,
		AwaitSwitchReady:     observer.awaitReady,
		OnSwitchProgress:     observer.changed,
...
func (w *Worker) StartNetwork() error {
	...
	if w.ECMCore != nil {
		w.ECMCore.SetConfig(w.Config.APN, w.Config.IPVersion)
	}
	return nc.Connect()
}
