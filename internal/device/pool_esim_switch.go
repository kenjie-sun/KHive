...
func (p *Pool) markESIMSwitchPhaseIfToken(deviceID string, token uint64, phase esim.SwitchPhase) bool {
	...
	p.switchMu.Unlock()
	if worker := p.GetWorker(deviceID); worker != nil && worker.EsimMgr != nil {
		worker.EsimMgr.UpdateSwitchProgress(logToken, phase)
	}
...
func (p *Pool) refreshPostSwitchIdentityWithPolling(deviceID string, worker *Worker, snapshot esimSwitchContext, pollTimeout time.Duration, pollInterval time.Duration) (bool, error) {
	...
	ctx := p.switchWorkerContext(worker)
	for {
	...
	newICCIDKey := normalizeSIMIdentityForCompare(newICCID)
	if iccidErr != nil || imsiErr != nil || newICCID == "" || newIMSI == "" {
		err := fmt.Errorf("post_switch_live_identity_incomplete")
		worker.MarkSIMIdentityDegraded("post_switch_finalize", err)
		return false, err
	}
	if targetICCIDKey != "" && newICCIDKey != targetICCIDKey {
...
func (p *Pool) newESIMSwitchCallbacks(deviceID string, workers ...*Worker) (func(esim.SwitchOperation, string) uint64, func(uint64), func(uint64, error), func(uint64, esim.SwitchPhase, error), func(uint64, esim.SwitchPhase)) {
	isCurrent := func() bool { return len(workers) == 0 || p.switchWorkerStillCurrent(workers[0]) }
...
func (p *Pool) schedulePostSwitchIdentityRefreshes(deviceID string, snapshot esimSwitchContext) {
	...
	for _, delay := range delays {
		...
		go func() {
			...
			worker := p.GetWorker(deviceID)
			if worker == nil || worker.Backend == nil || (snapshot.WorkerGeneration != 0 && worker.generation != snapshot.WorkerGeneration) || !worker.SIMIdentityConvergenceMatches(snapshot.TargetICCID, snapshot.IdentityGeneration) {
				return
			}
...
func (p *Pool) waitPostSwitchSIMAuthReady(deviceID string, worker *Worker) error {
	...
	waitCtx, cancel := context.WithTimeout(p.switchWorkerContext(worker), postSwitchSIMAuthReadyWaitTimeout)
	defer cancel()
...
func (p *Pool) restorePostSwitchConnectivity(deviceID string, worker *Worker, snapshot esimSwitchContext, restoreGateErr error, markDegraded bool) error {
	var restoreErr = restoreGateErr
	if restoreGateErr != nil {
		// The profile write may have succeeded, but the new SIM identity is
		// unknown. Never enable data using the previous card's preference.
		snapshot.NetworkEnabledBefore = false
		snapshot.QMIConnectedBefore = false
	}
	if worker.Config.VoWiFiEnabled {
		if restoreErr == nil {
			p.clearDesiredVoWiFiRecoverState(deviceID)
			restoreErr = p.voWiFiHost().SwitchEnd(p.switchWorkerContext(worker), deviceID, true)
			if restoreErr == nil {
				return nil
			}
		}
		...
		logger.Warn("切卡后 VoWiFi 恢复未完成", "device", deviceID, "err", restoreErr)
	}
	return errors.Join(restoreErr, p.restoreRadioDataForSwitchSnapshot(deviceID, worker, snapshot, "post_switch_finalize", worker.Config.ESIMSwitch.RadioCycle))
}
...
func (p *Pool) restoreRadioDataForSwitchSnapshot(deviceID string, worker *Worker, snapshot esimSwitchContext, reason string, radioCycleAlreadyDone bool) error {
	...
	if snapshot.FlightModeBefore {
		...
		if nc := worker.NetworkController(); nc != nil && (nc.IsConnected() || worker.ECMCore != nil) {
...
func (p *Pool) handleESIMSwitchAfter(deviceID string, token uint64) {
	...
	defer func() {
		if finalizeOK {
			p.markESIMSwitchPhaseIfToken(deviceID, token, esim.SwitchPhaseConnectivityCheck)
		}
	}()
	...
	p.markESIMSwitchPhaseIfToken(deviceID, token, esim.SwitchPhaseSIMInitializing)
	convergence := p.runPostSwitchConvergence(deviceID, token, worker, snapshot)
	if !p.switchWorkerStillCurrent(worker) || !p.switchTokenStillCurrent(deviceID, token, "sim_convergence") {
		return
	}
	if convergence.Degraded {
		phase, code := esim.SwitchPhaseSIMInitializing, "sim_init_failed"
		if convergence.Reason == "live_identity_not_ready" || convergence.Reason == "identity_convergence_timeout" {
			phase, code = esim.SwitchPhaseIdentityRefresh, "identity_unconfirmed"
		}
		p.reportESIMSwitchFailure(worker, token, phase, code)
	...
	if identityRefreshErr != nil || !identityReady {
		if identityRefreshErr == nil {
			identityRefreshErr = fmt.Errorf("target SIM identity is not confirmed")
		}
		p.reportESIMSwitchFailure(worker, token, esim.SwitchPhaseIdentityRefresh, "identity_unconfirmed")
		p.schedulePostSwitchIdentityRefreshes(deviceID, snapshot)
		p.restorePostSwitchConnectivity(deviceID, worker, snapshot, identityRefreshErr, false)
		return
	}
	// Successful switches follow the newly active card. The old snapshot is
	// only a fallback when no policy resolver exists, never an override of a
	// new card's network/flight preferences.
	p.markESIMSwitchPhaseIfToken(deviceID, token, esim.SwitchPhasePolicyApplying)
	if result := p.resolveAndApplyPolicy(worker, "esim_switched"); result.Applied {
		snapshot = postSwitchSnapshotForPolicy(snapshot, worker)
	}
	p.schedulePostSwitchIdentityRefreshes(deviceID, snapshot)
	...
	phase := esim.SwitchPhaseNetworkRestore
	if worker.Config.VoWiFiEnabled {
		phase = esim.SwitchPhaseVoWiFiRestore
	}
	p.markESIMSwitchPhaseIfToken(deviceID, token, phase)
	if !p.switchTokenStillCurrent(deviceID, token, "vowifi_restore") {
		return
	}
	if err := p.restorePostSwitchConnectivity(deviceID, worker, snapshot, restoreGateErr, true); err != nil {
		p.reportESIMSwitchFailure(worker, token, phase, "network_restore_failed")
		return
	}
...
func postSwitchSnapshotForPolicy(snapshot esimSwitchContext, worker *Worker) esimSwitchContext {
	snapshot.FlightModeBefore = worker.Config.AirplaneEnabled || worker.Config.VoWiFiEnabled
	snapshot.NetworkEnabledBefore = worker.Config.NetworkEnabled
	snapshot.QMIConnectedBefore = false
	snapshot.VoWiFiActiveBefore = worker.Config.VoWiFiEnabled
	return snapshot
}
