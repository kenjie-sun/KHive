...
func TestKHiveFailedIMSCanEnterRecovery(t *testing.T) {
	for _, tc := range []struct {
		state runtimehost.State
		want  bool
	}{
		{runtimehost.State{}, false},
		{runtimehost.State{LastErrorClass: "ims", LastReason: "ims_connection_lost"}, true},
		{runtimehost.State{LastErrorClass: "ims", IMSReady: true, SMSReady: true}, false},
	} {
		if got := failedRuntimeRecoverable(tc.state); got != tc.want {
			t.Fatalf("state=%+v got=%v", tc.state, got)
		}
	}
}
