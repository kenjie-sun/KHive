...
func TestKHiveDownloadUsesJSONBodyAndPreservesSSE(t *testing.T) {
	gin.SetMode(gin.TestMode)
	p := device.NewPool(&config.Config{})
	mgr := newTestEsimManager()
	mgr.StopSwitchRecovery() // Valid input must reach the manager without touching hardware.
	setNestedPrivateField(t, p, []string{"workers"}, map[string]*device.Worker{"dev-esim": {ID: "dev-esim", EsimMgr: mgr}})
	server := &Server{pool: p}
	for _, tc := range []struct {
		body   string
		status int
	}{
		{`{"smdp":"example.com","matching_id":"test-secret","confirmation_code":"test-confirm","aid_hex":"A000","imei":"490154203237518"}`, http.StatusOK},
		{`{}`, http.StatusBadRequest},
		{`{invalid`, http.StatusBadRequest},
	} {
		r := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(r)
		c.Params = gin.Params{{Key: "device_id", Value: "dev-esim"}}
		c.Request = httptest.NewRequest(http.MethodPost, "/devices/dev-esim/esim/actions/download", strings.NewReader(tc.body))
		c.Request.Header.Set("Content-Type", "application/json")
		server.handleEsimDownloadProfile(c)
		if r.Code != tc.status {
			t.Fatalf("status=%d body=%s", r.Code, r.Body.String())
		}
		if tc.status == http.StatusOK && (!strings.Contains(r.Body.String(), `"step":"error"`) || !strings.Contains(r.Body.String(), "设备连接已失效")) {
			t.Fatal(r.Body.String())
		}
		if strings.Contains(r.Body.String(), "test-secret") || strings.Contains(r.Body.String(), "test-confirm") {
			t.Fatal("credential echoed")
		}
	}
	if got := esimDeleteHTTPStatus(esim.NewDeleteProfileError(esim.DeleteProfileErrorActive, "active", nil)); got != http.StatusUnprocessableEntity {
		t.Fatal(got)
	}
}
