...
type deviceMgmtOverviewLiteItem struct {
	ESIMSession            string               `json:"esim_session"`
	SwitchProgress         *esim.SwitchProgress `json:"switch_progress,omitempty"`
	ID                     string               `json:"id"`
	Name                   string               `json:"name"`
	Running                bool                 `json:"running"`
	Healthy                bool                 `json:"healthy"`
...
func (s *Server) buildOverviewLiteItemFromWorkerWithModem(w *device.Worker, cfg config.DeviceConfig, status modem.DeviceStatus, radioLiveOK *bool, modemStatus modem.DeviceStatus) deviceMgmtOverviewLiteItem {
	...
	item.ESIMSession = w.EsimMgr.SessionID()
	item.SwitchProgress = w.EsimMgr.SwitchProgress()
	if nc := w.NetworkController(); nc != nil {
		item.PrivateIP = nc.GetPrivateIP()
		item.PrivateIPv6 = nc.GetPrivateIPv6()
	}
...
type overviewStreamEmitVersion struct {
	ESIMSession        string
	SwitchFailurePhase esim.SwitchPhase
	SwitchFailureCode  string
	SwitchToken        uint64
	SwitchPhase        esim.SwitchPhase
	SwitchBusy         bool
	VoWiFiActive       bool
...
func newOverviewStreamEmitVersion(item deviceMgmtOverviewLiteItem) overviewStreamEmitVersion {
	...
	v.ESIMSession = item.ESIMSession
	if item.SwitchProgress != nil {
		v.SwitchFailurePhase = item.SwitchProgress.FailurePhase
		v.SwitchFailureCode = item.SwitchProgress.FailureCode
		v.SwitchToken = item.SwitchProgress.Token
		v.SwitchPhase = item.SwitchProgress.Phase
		v.SwitchBusy = item.SwitchProgress.Busy
	}
...
func esimDeleteHTTPStatus(err error) int {
	...
	case esim.ClassifyDeleteProfileError(err) == esim.DeleteProfileErrorActive:
...
func (s *Server) handleEsimDownloadProfile(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil || strings.TrimSpace(req.SMDP) == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "请提交 JSON 参数，smdp 为必填项"})
		return
	}
	smdp, matchingID, confirmationCode, aidHex, imei := req.SMDP, req.MatchingID, req.ConfirmationCode, req.AIDHex, req.IMEI
