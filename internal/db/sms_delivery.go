...
const (
	...
	SMSDeliveryStateUnconfirmed = "unconfirmed"
)
...
func RecomputeSMSDelivery(messageID string, at time.Time) error {
	...
	failErr := DB.Model(&SMSDeliveryPart{}).
		Where("message_id = ? AND state = ?", messageID, SMSDeliveryPartStateFailed).
	...
	if failErr == nil {
		...
		var missingReports int64
		if err := DB.Model(&SMSDeliveryPart{}).Where("message_id = ? AND state = ?", messageID, SMSDeliveryPartStateTimeout).Count(&missingReports).Error; err != nil {
			return err
		}
		...
		if missingReports > 0 {
			state = SMSDeliveryStateUnconfirmed
			lastError = "submit report not received"
		} else if acked == int64(delivery.PartsTotal) {
...
func GetSMSDeliveryStatus(messageID string) (*SMSDeliveryStatus, error) {
	...
	// On read, expire missing submit reports even after a process restart.
	now := time.Now()
	expired := DB.Model(&SMSDeliveryPart{}).Where("message_id = ? AND state = ? AND sent_at < ?", messageID, SMSDeliveryPartStatePending, now.Add(-2*time.Minute)).Updates(map[string]any{"state": SMSDeliveryPartStateTimeout, "error_text": "submit report timeout", "updated_at": now})
	if expired.Error != nil {
		return nil, expired.Error
	}
	if expired.RowsAffected > 0 || (delivery.State == SMSDeliveryStateFailed && delivery.LastError == "submit report timeout") {
		if err := RecomputeSMSDelivery(messageID, now); err != nil {
			return nil, err
		}
		if err := DB.Where("message_id = ?", messageID).First(&delivery).Error; err != nil {
			return nil, err
