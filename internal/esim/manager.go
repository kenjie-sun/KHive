...
type Manager struct {
	...
	profileWriteInProgress      bool         // Protected by switchProgressMu.
	switchInProgress            atomic.Bool  // Covers the asynchronous recovery, not just EnableProfile.
	switchProgressMu            sync.RWMutex
	switchProgress              *SwitchProgress
	switchLifecycleOnce         sync.Once
	switchContext               context.Context
	cancelSwitchLifecycle       context.CancelFunc
	switchSessionID             string
	awaitSwitchReady            func(context.Context) error
	onSwitchProgress            func()
...
const (
	...
	SwitchPhaseSIMInitializing     SwitchPhase = "sim_initializing"
	SwitchPhasePolicyApplying      SwitchPhase = "policy_applying"
	SwitchPhaseNetworkRestore      SwitchPhase = "network_restore"
	SwitchPhaseConnectivityCheck   SwitchPhase = "connectivity_check"
...
func newLPAClientWithChannel(ch driver.SmartCardChannel, aid []byte) (*lpa.Client, error) {
	// The dependency logs complete HTTP/APDU payloads at debug level. Keep
	// protocol secrets out of logs regardless of the application log level.
	opts := &lpa.Options{Channel: ch, AID: aid, MSS: 120, Logger: slog.New(slog.NewTextHandler(io.Discard, nil))}
...
func NewManager(opts ManagerOptions) (*Manager, error) {
	...
	mgr := &Manager{
		...
		awaitSwitchReady:     opts.AwaitSwitchReady,
		onSwitchProgress:     opts.OnSwitchProgress,
	...
	case transportAT:
		...
		mgr.smartCardChannelFactory = func() (driver.SmartCardChannel, error) {
			ch := NewModemChannel(opts.Modem)
			ch.useCSIM = opts.ATUseCSIM
			return ch, nil
		}
...
func (m *Manager) emitSwitchPhase(operation SwitchOperation, token uint64, phase SwitchPhase) {
	...
	m.UpdateSwitchProgress(token, phase)
	if m.onSwitchPhase != nil {
...
func cloneProfiles(groups []EUICCProfiles) []EUICCProfiles {
	...
	for i, group := range groups {
		cloned[i] = EUICCProfiles{
			EID:      group.EID,
			AIDHex:   group.AIDHex,
			Profiles: make([]ProfileItem, 0, len(group.Profiles)),
		}
...
func (m *Manager) SwitchProfileWithResult(ctx context.Context, targetICCID string, aidHex string) (SwitchProfileResult, error) {
	...
	if !m.beginSwitchProgress(operation, targetICCID) {
		if m.SwitchContext().Err() != nil {
			return result, fmt.Errorf("设备连接已失效，请等待重新上线")
		}
	...
	if errors.As(enableErr, &operationErr) && operationErr.Operation == sgp22.EnableProfile &&
		operationErr.Result == sgp22.ProfileOperationResultProfileNotInDisabledState {
		group, readErr := m.loadProfileGroupForAIDFresh(targetAID)
		if readErr == nil {
			for _, profile := range group.Profiles {
				if profile.State == int(sgp22.ProfileEnabled) && isTargetICCIDActive(targetICCID, profile.ICCID) {
					m.replaceCachedProfileGroup(group)
					enableErr = nil
					logger.Info("目标 Profile 已启用且卡内状态已确认，继续恢复运行态", "device", m.deviceID)
					break
				}
			}
		}
	...
	go func() {
		defer m.finishSwitchProgress()
		m.runPostSwitchRecovery(operation, switchToken, targetICCID)
	}()
...
func (m *Manager) DisableProfile(ctx context.Context, targetICCID string, aidHex string) error {
	...
	if !m.beginSwitchProgress(operation, targetICCID) {
		if m.SwitchContext().Err() != nil {
			return fmt.Errorf("设备连接已失效，请等待重新上线")
		}
		return ErrOperationInProgress
	}
	m.opMu.Lock()
	...
	go func() {
		defer m.finishSwitchProgress()
		m.runPostSwitchHook(operation, switchToken)
		if progress := m.SwitchProgress(); progress != nil && progress.Phase != SwitchPhaseFailed && progress.Phase != SwitchPhaseDegraded {
			m.UpdateSwitchProgress(switchToken, SwitchPhaseDone)
		}
	}()
...
func (m *Manager) runPostSwitchRecovery(operation SwitchOperation, token uint64, targetICCID string) {
	...
	hookStart, postSwitchDelayMS := m.waitPostSwitchHookDelay()
	m.finishPostSwitchHook(operation, token, hookStart, postSwitchDelayMS)
	progress := m.SwitchProgress()
	if progress != nil && (progress.Phase == SwitchPhaseFailed || progress.Phase == SwitchPhaseDegraded) {
		return
	}
	if m.awaitSwitchReady != nil {
		m.UpdateSwitchProgress(token, SwitchPhaseConnectivityCheck)
		ctx, cancel := context.WithTimeout(m.SwitchContext(), 45*time.Second)
		err := m.awaitSwitchReady(ctx)
		cancel()
		if err != nil {
			m.ReportSwitchFailure(token, SwitchPhaseConnectivityCheck, "network_ready_timeout")
			logger.Warn("切卡指令已执行，但网络就绪确认未通过", "device", m.deviceID, "err", err)
			return
		}
	}
	m.UpdateSwitchProgress(token, SwitchPhaseDone)
}
...
func (m *Manager) finishPostSwitchHook(operation SwitchOperation, token uint64, hookStart time.Time, postSwitchDelayMS int64) {
	...
	if m.onAfterSwitch != nil && m.SwitchContext().Err() == nil {
...
func downloadNotificationResult(observed bool, retrieveErr error, handleErr error) DownloadProfileResult {
	...
	var cleanupErr *notificationCleanupError
	if errors.As(handleErr, &cleanupErr) {
		return DownloadProfileResult{Warning: "Profile 下载完成，通知已发送，但卡内通知记录清理失败", WarningCode: "download_notification_cleanup_failed"}
	}
	if handleErr != nil {
		return DownloadProfileResult{
			Warning:     "Profile 下载完成，但通知未完全确认",
...
func deleteNotificationResult(observed bool, retrieveErr error, handleErr error) DeleteProfileResult {
	...
	var cleanupErr *notificationCleanupError
	if errors.As(handleErr, &cleanupErr) {
		return DeleteProfileResult{Warning: "Profile 已删除，通知已发送，但卡内通知记录清理失败", WarningCode: "delete_notification_cleanup_failed"}
	}
	if handleErr != nil {
		return DeleteProfileResult{
			Warning:     "Profile 已删除，但删除通知发送未完全确认",
...
func (m *Manager) RetryNotification(sequenceNumber int64, aidHex string) error {
	...
	if err := sendAndRemoveNotification(client, seq, pendingNotifications, 300*time.Millisecond); err != nil {
		return NewNotificationError(NotificationErrorInternal, fmt.Sprintf("通知发送或收尾失败: %v", err), err)
	}
	return nil
}
...
func (m *Manager) DownloadProfile(ctx context.Context, aidHex, smdp, matchingID, confirmationCode, downloadIMEI string, progressFn DownloadProgressFn) (DownloadProfileResult, error) {
	...
	release, lockErr := m.tryProfileWrite()
	if lockErr != nil {
		return DownloadProfileResult{}, lockErr
	}
	defer release()
	writeStarted := time.Now()
	defer func() {
	...
	stopLifecycle := context.AfterFunc(m.SwitchContext(), cancelDownload)
	defer stopLifecycle()
	...
	beforeFreeNvramBytes := int32(0)
	{
		...
		logger.Info("eUICC 空间预检", "device", m.deviceID, "freeNvram", checkInfo.FreeNvram, "space_known", beforeFreeNvramBytes > 0)
	}
	...
	parsedURL, err := parseDownloadServer(smdp)
	if err != nil {
		return DownloadProfileResult{}, err
	}
	...
	logger.Info("开始下载 eSIM profile",
		"device", m.deviceID,
		"smdp", parsedURL.Host,
		"has_matching_id", strings.TrimSpace(matchingID) != "",
		"AID", aidHex)
	...
	// The LPA can return (nil, nil) after a successful cancellation.
	if err == nil && downloadResult == nil {
		err = fmt.Errorf("下载未返回安装结果，请刷新卡列表核实后再决定是否重试")
	}
	if err != nil {
		downloadErr := NewDownloadProfileError(err)
		downloadErr.Message = redactDownloadSecrets(downloadErr.Message, matchingID, confirmationCode)
		downloadErr.Details = redactDownloadSecrets(downloadErr.Details, matchingID, confirmationCode)
		downloadErr.OriginalMessage = redactDownloadSecrets(downloadErr.OriginalMessage, matchingID, confirmationCode)
		logger.Warn("下载 eSIM profile 失败",
			...
			"err", downloadErr.OriginalMessage)
		if installStarted && preDownloadNotificationsErr == nil {
			...
			client = nil
			if recovered, ok := m.recoverDownloadInstallFinalizeError(ctx, targetAID, preDownloadNotifications, fmt.Errorf("%s", downloadErr.OriginalMessage)); ok {
				result = recovered
...
func (m *Manager) DeleteProfile(targetICCID string, aidHex string) (DeleteProfileResult, error) {
	release, lockErr := m.tryProfileWrite()
	if lockErr != nil {
		return DeleteProfileResult{}, lockErr
	}
	defer release()
	writeStarted := time.Now()
	defer func() {
	...
	defer func() {
		if client != nil {
			_ = m.closeLPAClientForOperation("delete_profile", client)
		}
	}()

	if err := validateDeleteTarget(client, iccid); err != nil {
		return result, err
	}
	...
	// Invalidate even when the transport response is ambiguous: the card may
	// already have committed the deletion. Never issue DeleteProfile twice.
	defer m.invalidateOverviewCache("delete_profile_attempt")
	recovered := false
	if deleteErr := client.DeleteProfile(iccid); deleteErr != nil {
		_ = m.closeLPAClientForOperation("delete_profile_reopen", client)
		client = nil
		var recoveryErr error
		if nerr == nil {
			client, recoveryErr = m.recoverDeleteProfileClient(targetAID, iccid, lastSeq)
		} else {
			recoveryErr = fmt.Errorf("删除前通知基线读取失败: %w", nerr)
		}
		if recoveryErr != nil {
			return DeleteProfileResult{}, NewDeleteProfileError(DeleteProfileErrorInternal,
				fmt.Sprintf("删除响应异常，尚不能确认最终结果；请刷新卡列表，勿直接重复删除: %v (核验: %v)", deleteErr, recoveryErr), deleteErr)
		}
		recovered = true
		logger.Warn("删除响应异常，但新会话已确认目标消失并生成删除通知，继续收尾", "device", m.deviceID, "err", deleteErr)
	}
	...
	result = m.resolveDeleteNotificationResult(func() ([]*sgp22.NotificationMetadata, error) {
		...
		return sendAndRemoveNotification(client, seq, pendingNotifications, 300*time.Millisecond)
	})
	...
	if recovered && result.WarningCode == "" {
		result.Warning = "删除响应异常，但已重新读卡确认删除成功，删除通知已发送并清理"
		result.WarningCode = "delete_apdu_error_recovered"
	}
...
func (m *Manager) recoverDownloadInstallFinalizeError(ctx context.Context, targetAID []byte, preDownloadNotifications []*sgp22.NotificationMetadata, downloadErr error) (DownloadProfileResult, bool) {
	...
	if notificationResult.WarningCode == "download_notification_cleanup_failed" {
		// Installation and delivery are confirmed; failed local cleanup must
		// not turn this into a failed download and invite another installation.
		logger.Warn("下载收尾异常，但安装通知已发送；仅卡内通知清理失败", "device", m.deviceID, "warning_code", notificationResult.WarningCode)
