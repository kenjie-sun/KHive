...
func TestResolveDownloadIMEIUsesExplicitCustomIMEIBeforeProviders(t *testing.T) {
	...
	imei, err := mgr.resolveDownloadIMEI(context.Background(), "490154203237518")
	if err != nil {
...
func (c *lifecycleSmartCardChannel) Transmit(command []byte) ([]byte, error) {
	...
	// Notification finalization now sends ES10b.RemoveNotificationFromList.
	if len(command) > 6 && bytes.Equal(command[5:7], []byte{0xBF, 0x30}) {
		return []byte{0xBF, 0x30, 0x03, 0x80, 0x01, 0x00, 0x90, 0x00}, nil
	}
