...
const (
	...
	trafficCounterSourceInterface = "interface"
...
func (s *Sampler) poolWorkerInterfaces() []workerInterface {
	...
	for _, w := range workers {
		...
		if worker.ECMCore != nil {
			source = trafficCounterSourceInterface
			readCounters = func(ctx context.Context) (trafficCounters, error) { return readInterfaceTrafficCounters(ctx, iface) }
		}
		out = append(out, workerInterface{id: id, iface: iface, source: source, networkReady: networkReady, readCounters: readCounters})
	}
	return out
}
