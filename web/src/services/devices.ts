...
import { normalizeProfileGroups } from '../components/deviceEsimOptimistic'
...
export const devicesService = {
  ...
  getEsimOverview(id: string, options?: { refresh?: boolean; signal?: AbortSignal }) {
    return callService(async () => {
      ...
      return {
        ...
        profiles: normalizeProfileGroups(res.data?.profiles)
      }
    })
  },
  getEsimProfiles(id: string, options?: { refresh?: boolean; signal?: AbortSignal }) {
    return callService(async () => {
      ...
      return normalizeProfileGroups(res.data)
    })
  },
  ...
  switchEsimProfile(id: string, payload: { iccid: string; aid_hex: string }) {
    return callService(async () => {
      const res = await api.post<{ switch_token: number }>(`/devices/${id}/esim/actions/switch`, payload)
      return res.data
    })
  },
  ...
  deleteEsimProfile(id: string, iccid: string, aidHex: string) {
    return callService(async () => {
      const res = await api.delete<DeleteEsimProfileResponse>(`/devices/${id}/esim/profiles/${iccid}`, {
  ...
  downloadEsimProfile(id: string, payload: { smdp: string; matching_id?: string; confirmation_code?: string; aid_hex?: string; imei?: string }) {
    return callService(async () => {
      await api.post(`/devices/${id}/esim/actions/download`, payload)
      return true
    })
  },
