...
function normalizeICCID(value: string | undefined | null): string {
  if (!value) return ''
  return value.replace(/\s+/g, '').trim().toUpperCase().replace(/F+$/, '')
}

// Older servers serialize empty eUICC partitions as null.
export function normalizeProfileGroups(groups: EsimEUICCProfiles[] | null | undefined): EsimEUICCProfiles[] {
  return (groups ?? []).map(group => ({ ...group, profiles: group.profiles ?? [] }))
}
...
export function applyOptimisticActiveState(
  ...
  groups = normalizeProfileGroups(groups)
