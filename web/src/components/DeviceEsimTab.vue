...
import type { EsimChipInfo, EsimEUICCProfiles, EsimNotificationItem, EsimSpaceDelta, EsimSwitchProgress } from '../types/api'
import { parseEsimActivationCode, readEsimQrPhoto } from './deviceEsimQr'
...
import { switchProgressNotice, pendingSwitchSettled, switchSessionChanged } from './deviceEsimProgress'
...
const props = defineProps<{
  ...
  currentIccid?: string
  switchProgress?: EsimSwitchProgress | null
  esimSession?: string
}>()

const emit = defineEmits<{ policyChanged: [] }>()
const openPolicy = ref('')
...
const pendingSwitchToken = ref<number | null>(null)
const interruptedSwitch = ref(false)
const switchBusy = computed(() => props.deviceOnline !== true || switching.value !== null || props.switchProgress?.busy === true)
const switchNotice = computed(() => {
  if (interruptedSwitch.value && !props.switchProgress) return { title: '设备连接已重建', description: '上一次切卡已中断，请核对当前卡和网络状态。', type: 'warning' as const }
  return switchProgressNotice(props.switchProgress)
})
watch(() => props.switchProgress, progress => {
  if (pendingSwitchSettled(progress, pendingSwitchToken.value)) {
    switching.value = null
    pendingSwitchToken.value = null
    void fetchOverview()
  }
}, { deep: true })
watch(() => props.esimSession, (session, previous) => {
  if (!switchSessionChanged(previous, session)) return
  interruptedSwitch.value = switching.value !== null || interruptedSwitch.value
  switching.value = null
  pendingSwitchToken.value = null
  if (props.isActive) void fetchOverview()
})
watch(() => props.deviceId, () => { switching.value = null; pendingSwitchToken.value = null; interruptedSwitch.value = false })
...
const profileWriteBusy = computed(() => switchBusy.value || downloading.value || deleting.value !== null)
...
async function importQrPhoto(event: Event) {
  const input = event.target as HTMLInputElement
  const file = input.files?.[0]
  input.value = '' // Allow selecting the same photo again after an error.
  if (!file || profileWriteBusy.value) return
  cancelQrRead()
  const controller = new AbortController()
  qrController = controller
  qrReading.value = true
  try {
    const parsed = await readEsimQrPhoto(file, controller.signal)
    if (controller.signal.aborted || profileWriteBusy.value) return
    downloadForm.value.smdp = parsed.smdp
    downloadForm.value.matchingId = parsed.matchingId
    downloadForm.value.confirmationCode = ''
    downloadError.value = ''
    qrNotice.value = parsed.confirmationRequired
      ? '二维码已识别。请填写运营商提供的确认码，再点击“开始下载”。'
      : '二维码已识别，请核对目标 eUICC 后点击“开始下载”。'
  } catch (error) {
    if (!controller.signal.aborted) {
      qrError.value = error instanceof Error ? error.message : '二维码识别失败，请更换图片重试'
    }
  } finally {
...
watch([() => props.deviceId, () => props.esimSession, () => props.isActive, () => props.deviceOnline], cancelQrRead)

// Pasted activation codes use the same validation as QR photos.
watch(() => downloadForm.value.smdp, (newVal) => {
  const value = newVal.trim()
  if (value.startsWith('LPA:')) {
    try {
      const parsed = parseEsimActivationCode(value)
      downloadForm.value.smdp = parsed.smdp
      downloadForm.value.matchingId = parsed.matchingId
      ElMessage.success('已自动解析完整的 LPA 激活码')
    } catch {
      ElMessage.warning('eSIM 激活码格式不正确，请检查后重试')
    }
...
async function switchProfile(iccid: string, currentState: number, aidHex: string) {
  if (profileWriteBusy.value || currentState === 1) return
  const deviceId = props.deviceId
  const session = props.esimSession
  const action = '启用'
  ...
  if (!confirmed || profileWriteBusy.value || props.deviceId !== deviceId || switchSessionChanged(session, props.esimSession)) return

  switching.value = iccid
  interruptedSwitch.value = false
  try {
    const result = await devicesService.switchEsimProfile(deviceId, {
      iccid,
      aid_hex: aidHex
    })
    if (props.deviceId !== deviceId || switchSessionChanged(session, props.esimSession)) return
    if (!result.ok) throw new Error(result.error.message || `${action}失败`)
    pendingSwitchToken.value = result.data.switch_token
    if (pendingSwitchSettled(props.switchProgress, pendingSwitchToken.value)) {
      switching.value = null
      pendingSwitchToken.value = null
    }
    applyOptimisticActive(iccid, aidHex)
    ElMessage.success('切卡指令已提交，设备正在恢复网络')
  } catch (e: unknown) {
    if (props.deviceId !== deviceId || switchSessionChanged(session, props.esimSession)) return
    switching.value = null
    pendingSwitchToken.value = null
...
async function saveRename(iccid: string, aidHex: string) {
  if (profileWriteBusy.value) return
...
async function deleteProfile(iccid: string, name: string, aidHex: string) {
  ...
  if (input !== last4 || profileWriteBusy.value || props.deviceId !== deviceId || switchSessionChanged(session, props.esimSession)) return
...
async function downloadProfile() {
  if (profileWriteBusy.value || qrReading.value) return
  const deviceId = props.deviceId
  const session = props.esimSession
  const sessionCurrent = () => props.deviceId === deviceId && !switchSessionChanged(session, props.esimSession)
  ...
  const payload = { smdp, matching_id: matchingId, confirmation_code: confirmationCode, aid_hex: targetAidHex, imei: imei.trim() }
  const base = api.defaults.baseURL || ''
  const url = `${base}/devices/${deviceId}/esim/actions/download`
  ...
  try {
    ...
    outer: while (true) {
      ...
      if (done) break
      if (!sessionCurrent()) throw new Error('设备会话已改变，请重新读取卡列表核实安装结果，勿直接重复下载。')
    ...
    await reader.cancel()
    if (!terminal) throw new Error('下载连接已结束但未收到最终结果，请刷新卡列表及通知核实，勿直接重复下载。')
  } catch (e: unknown) {
...
<template>
  <div class="space-y-5">
    <el-alert v-if="switchNotice" :title="switchNotice.title" :description="switchNotice.description" :type="switchNotice.type" :closable="false" show-icon aria-live="polite" />
    ...
    <template v-else>
      ...
      <div v-else class="divide-y divide-gray-100 dark:divide-white/10">
        <div
          ...
          :key="p.iccid"
          class="px-4 py-3 flex flex-wrap items-center justify-between gap-3 hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors"
        >
          ...
          <div v-if="renaming !== p.iccid" class="flex items-center gap-2 flex-shrink-0">
            ...
            <el-button size="small" plain :aria-expanded="openPolicy === group.aid_hex + ':' + p.iccid" @click="openPolicy = openPolicy === group.aid_hex + ':' + p.iccid ? '' : group.aid_hex + ':' + p.iccid">
              策略
            </el-button>
            <el-button
              size="small"
              ...
              :disabled="profileWriteBusy || p.state === 1"
          ...
          <EsimCardPolicyInline
            v-if="openPolicy === group.aid_hex + ':' + p.iccid"
            :device-id="deviceId"
            :iccid="p.iccid"
            :current-iccid="currentIccid"
            :device-online="deviceOnline === true"
            @policy-changed="emit('policyChanged')"
          />
        </div>
      </div>
    </div>
      ...
      <div class="mb-4 space-y-2">
        <input ref="qrFileInput" type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/bmp" class="hidden" aria-label="选择 eSIM 二维码照片" :disabled="profileWriteBusy || qrReading" @change="importQrPhoto" />
        <div class="flex flex-wrap items-center gap-2">
          <el-button :loading="qrReading" :disabled="profileWriteBusy || qrReading" @click="qrFileInput?.click()">
            {{ qrReading ? '正在识别二维码' : '上传二维码照片' }}
          </el-button>
          <span class="text-xs text-gray-500 dark:text-gray-400">支持 JPG、PNG、WebP，最大 10 MB</span>
        </div>
        <p class="text-xs text-gray-500 dark:text-gray-400">照片仅在当前浏览器中识别，也可直接填写下方信息。</p>
        <p v-if="qrError" role="alert" class="text-xs text-red-500">{{ qrError }}</p>
        <p v-else-if="qrNotice" role="status" class="text-xs text-emerald-600 dark:text-emerald-400">{{ qrNotice }}</p>
      </div>
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
        ...
        <div class="space-y-1">
          ...
          <el-input v-model="downloadForm.confirmationCode" type="password" show-password placeholder="可选" />
        </div>
        <div class="space-y-1">
      ...
      <div class="flex justify-end mt-4">
        <el-button type="primary" :loading="downloading" :disabled="profileWriteBusy || qrReading" @click="downloadProfile" class="!border-0">
