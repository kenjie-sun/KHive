...
async function doCheckUpdate() {
  ...
  try {
    ...
    if (res.data.enabled === false) {
      ElMessage.info(res.data.disabled_reason || '请使用 KHive 构建包手动更新')
...
async function doApplyUpdate() {
  if (!updateInfo.value || updateInfo.value.enabled === false) return

  if (updateInfo.value.is_docker) {
    ElMessageBox.alert(
      '检测到当前系统运行在 Docker 环境下。<br><br>不建议在 Docker 容器内直接执行文件热替换。请直接通过拉取最新镜像（如 <code>更新对应的 KHive 镜像</code>）并重启容器来完成升级！',
      '环境警告',
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <div class="ui-card p-8 relative overflow-hidden group">
         ...
         <div class="space-y-4 text-sm relative z-10">
            ...
            <div class="p-3 bg-gray-50 dark:bg-white/5 rounded-lg">
              <FieldRow label="项目" value="KHive · 基于 VoHive" />
            </div>
              ...
              <el-tab-pane label="Bark" name="bark" class="pt-2">
                ...
                <div class="space-y-4">
                  ...
                  <div class="space-y-1">
                    ...
                    <el-input v-model="barkSettings.group" :disabled="!barkSettings.enabled" placeholder="例如 khive" />
